...
thpool_t* thpool_init(int threadsN){
    ...
    tp_p->jobqueue->queueCond = (pthread_cond_t*)malloc(sizeof(pthread_cond_t));
    pthread_cond_init(tp_p->jobqueue->queueCond, &cat);
    tp_p->jobqueue->queueMutex = (pthread_mutex_t*)malloc(sizeof(pthread_mutex_t));
    pthread_mutex_init(tp_p->jobqueue->queueMutex, &mat);
...
void thpool_thread_do(thpool_t* tp_p){
	...
	while(thpool_keepalive){
        ...
        /*
		if (rc = sem_wait(tp_p->jobqueue->queueSem)) {// WAITING until there is work in the queue 
            if (errno == EINTR) {
                fprintf(stdout, "thpool semaphore intr: %d, %d\n", rc, errno);
                continue;
            }
            fprintf(stdout, "thpool_thread_do(): Waiting for semaphore, %d,%d", rc, errno);
			perror("thpool_thread_do(): Waiting for semaphore");
			exit(1);
		}
        */
        rc = pthread_mutex_lock(tp_p->jobqueue->queueMutex);
        fprintf(stdout, "before sem_wait222...%d\n", rc);
        rc = pthread_cond_wait(tp_p->jobqueue->queueCond, tp_p->jobqueue->queueMutex);
        fprintf(stdout, "after cond wait111...%d\n", rc);
        pthread_mutex_unlock(tp_p->jobqueue->queueMutex);
        fprintf(stdout, "after cond wait...\n");
...
void thpool_thread_do2(thpool_t* tp_p) {
...
void thpool_jobqueue_add(thpool_t* tp_p, thpool_job_t* newjob_p){ /* remember that job prev and next point to NULL */
    ...
    pthread_mutex_lock(tp_p->jobqueue->queueMutex);
    rc = pthread_cond_signal(tp_p->jobqueue->queueCond);
    pthread_mutex_unlock(tp_p->jobqueue->queueMutex);
