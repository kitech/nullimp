...
ngx_http_circle_gif_handler(ngx_http_request_t *r)
{
    ...
    cglcf = (ngx_http_circle_gif_loc_conf_t*)ngx_http_get_module_loc_conf(r, ngx_http_circle_gif_module);
    ...
    image = (unsigned char*)ngx_palloc(r->pool, cglcf->circle_sizes[radius_index]);
    if (image == NULL) {
...
ngx_http_circle_gif(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_http_circle_gif_loc_conf_t *cglcf = (ngx_http_circle_gif_loc_conf_t*)conf;
...
ngx_http_circle_gif_init(ngx_http_circle_gif_loc_conf_t *cglcf)
{
  ...
  if ((cglcf->circle_templates = (unsigned char**)malloc((1+radius2index(cglcf->max_radius, cglcf))*sizeof(unsigned char*))) == NULL ||
	  (cglcf->circle_sizes = (size_t*)malloc((1+radius2index(cglcf->max_radius, cglcf))*sizeof(size_t))) == NULL) {
    perror("malloc()");
    return NGX_ERROR;
  }
  for (i=0;i<=radius2index(cglcf->max_radius, cglcf);i++) {
      cglcf->circle_templates[i] = (unsigned char*)ngx_http_circle_gif_template(cglcf->min_radius+i*cglcf->step_radius, &cglcf->circle_sizes[i], 
...
ngx_http_circle_gif_create_loc_conf(ngx_conf_t *cf)
{
    ...
    conf = (ngx_http_circle_gif_loc_conf_t*)ngx_pcalloc(cf->pool, sizeof(ngx_http_circle_gif_loc_conf_t));
    if (conf == NULL) {
...
{
    ngx_http_circle_gif_loc_conf_t *prev = (ngx_http_circle_gif_loc_conf_t*)parent;
    ngx_http_circle_gif_loc_conf_t *conf = (ngx_http_circle_gif_loc_conf_t*)child;
